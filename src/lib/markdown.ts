  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("- ") || line.startsWith("* ")) {
      ...
      while (
        ...
        items.push(
          `<li class="mb-2 pl-1">${inline(lines[i].slice(2))}</li>`
        );
        i++;
      }
      out.push(
    ...
    // ===== 段落 =====
    out.push(
      `<p class="text-gray-700 leading-loose my-4">${inline(line)}</p>`
    );
    i++;
  }
...
function renderQuote(text: string): string {
  ...
  if (speakerMatch) {
    ...
    return `<div class="my-6 flex gap-3 items-start">
  <div class="shrink-0 w-12 h-12 rounded-full bg-orange-100 border-2 border-orange-300 flex items-center justify-center text-2xl">📒</div>
  <div class="flex-1 relative bg-orange-50 border-2 border-orange-200 rounded-xl px-4 py-3">
    <div class="absolute -left-2 top-3 w-0 h-0 border-t-8 border-t-transparent border-r-8 border-r-orange-200 border-b-8 border-b-transparent"></div>
    <div class="font-bold text-orange-700 text-xs mb-1">${inline(speaker.trim())}</div>
    <div class="text-gray-700 text-sm leading-relaxed">${inline(message.trim())}</div>
  </div>
</div>`;
  }
  // 通常の引用
...
function renderBox(tag: string, content: string): string {
  // ボックス内の内容をMarkdownとしてレンダリング（簡易）
  const innerHtml = renderMarkdown(content);

  if (tag === "recommended") {
    return `<div class="my-8 rounded-2xl border-2 border-orange-300 overflow-hidden shadow-sm">
  <div class="bg-orange-500 text-white px-5 py-3 font-bold flex items-center gap-2">
    <span>✅</span><span>こんな方におすすめ</span>
  </div>
  <div class="bg-orange-50 px-5 py-4 [&_ul]:my-2 [&_li]:text-gray-800">${innerHtml}</div>
</div>`;
  }

  if (tag === "point") {
    return `<div class="my-8 rounded-2xl border-2 border-blue-300 overflow-hidden shadow-sm">
  <div class="bg-blue-500 text-white px-5 py-3 font-bold flex items-center gap-2">
    <span>💡</span><span>ポイント</span>
  </div>
  <div class="bg-blue-50 px-5 py-4">${innerHtml}</div>
</div>`;
  }

  if (tag === "note") {
    return `<div class="my-8 rounded-2xl border-2 border-yellow-300 overflow-hidden shadow-sm">
  <div class="bg-yellow-500 text-white px-5 py-3 font-bold flex items-center gap-2">
    <span>⚠️</span><span>注意</span>
  </div>
  <div class="bg-yellow-50 px-5 py-4">${innerHtml}</div>
</div>`;
  }

  if (tag === "warning") {
    return `<div class="my-8 rounded-2xl border-2 border-red-300 overflow-hidden shadow-sm">
  <div class="bg-red-500 text-white px-5 py-3 font-bold flex items-center gap-2">
    <span>🚨</span><span>警告</span>
  </div>
  <div class="bg-red-50 px-5 py-4">${innerHtml}</div>
</div>`;
  }
